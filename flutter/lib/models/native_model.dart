...
class PlatformFFI {
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libdeskviewer.so')
        : isLinux
            ? DynamicLibrary.open('libdeskviewer.so')
            : isWindows
                ? DynamicLibrary.open('deskviewer.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("libdeskviewer.dylib") :
  ...
  void _startListenEvent(DeskviewerImpl deskviewerImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = deskviewerImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
